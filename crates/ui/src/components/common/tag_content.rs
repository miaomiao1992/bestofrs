...
pub fn TagContent(
    ...
    let item = use_resource({
        ...
        move || {
            let value = value.clone();
            async move { get_tag_with_meta_by_value(value, Some(top_n)).await.ok().flatten() }
        }
    });

    match item() {
        Some(Some(tag)) => rsx! {
            div { class: "flex h-full min-h-72 w-80 flex-col",
                div { class: "border-b border-primary-5 pb-3",
                    div { class: "flex items-center gap-2",
                        TagsIcon { width: 14, height: 14, class: "text-secondary-4" }
                    ...
                    p { class: "mt-2 line-clamp-2 text-xs leading-relaxed text-secondary-4",
                        "{tag.description.clone().unwrap_or_else(|| \"No description\".to_string())}"
                    }
                }
                div { class: "my-3 flex min-h-0 flex-1 flex-col border border-primary-5 bg-primary px-2 py-2",
                    ...
                    ul { class: "min-h-0 flex-1 space-y-1 overflow-y-auto",
                        if tag.top_repos.is_empty() {
                            li { class: "border border-primary-5 bg-primary-1 px-2 py-2 text-xs text-secondary-5",
                                "No repos"
                            }
                        } else {
                            for repo in tag.top_repos.into_iter() {
                                li {
                                    key: "{repo.repo_id}",
                                    class: "flex items-center gap-2 border border-primary-5 bg-primary-1 px-2 py-2",
                                    div { class: "h-6 w-6 shrink-0 overflow-hidden",
                                        RepoAvatar {
                                            repo_id: repo.repo_id.clone(),
                                            avatar_urls: repo.avatar_urls,
                                            size: AvatarImageSize::Custom,
                                            class: "h-6 w-6 overflow-hidden border border-primary-6 bg-primary",
                ...
                button {
                    r#type: "button",
                    class: "mt-1 w-full border border-primary-6 bg-primary-1 px-3 py-2 text-center text-xs font-mono uppercase tracking-wide text-secondary-3 transition-colors hover:bg-primary-2 hover:text-secondary-2",
                    onclick: move |evt| {
