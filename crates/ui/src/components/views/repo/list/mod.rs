...
use crate::components::common::{CommonPagination, IOCell, RepoRow};
    ...
    rsx! {
        div { class: "py-8 space-y-6",
            div { class: "rounded-xl border border-primary-6 bg-primary-2 px-6 py-6 space-y-6",
                div { class: "space-y-2",
                    h1 { class: "text-2xl font-semibold tracking-tight text-secondary-4", "Repo" }
                    p { class: "text-sm text-secondary-5", "All repos" }
                }
...
fn RepoListIO(mut page_size: Signal<u32>, mut current_page: Signal<u32>) -> Element {
    ...
    match repos() {
        Some(Ok(page)) => {
            ...
            rsx! {
                div { class: "space-y-4",
                    div { class: "rounded-lg border border-primary-6 bg-primary-1 px-4 py-3 text-sm font-medium text-secondary-4",
                        "total: {total}"
                        if total > 0 {
